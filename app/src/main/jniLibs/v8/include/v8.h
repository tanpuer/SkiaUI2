...
// <YUNOS> changed: begin
class Object; // work arround some closed-source addons compiling issue.
#ifdef ENABLE_JSAOT_
class JsoGenerator;
class JsoLoader;
#endif  // ENABLE_JSAOT_
...
class WeakCallbackInfo {
 public:
  ...
  // <ALIOS> adapt for webengin 5.7: begin
  V8_DEPRECATED("use indexed version")
  V8_INLINE void* GetInternalField1() const {
    return embedder_fields_[0];
  }
  V8_DEPRECATED("use indexed version")
  V8_INLINE void* GetInternalField2() const {
...
class V8_EXPORT Script {
 public:
  ...
  V8_DEPRECATE_SOON("Use maybe version")
  static Local<Script> Compile(Local<String> source, Local<String> file_name);
...
class V8_EXPORT ScriptCompiler {
 public:
   ...
   */
  // <ALIOS> adapt for webengin 5.7: begin
  V8_DEPRECATE_SOON("Source string is no longer required")
...
// <YUNOS> changed: begin
// for ENABLE_JSAOT_ and node snapshot
  /**
   * Clear script source.
   */
  static void ClearScriptSource(
      Isolate* isolate, MaybeLocal<UnboundScript>* compile_result);
// <YUNOS> changed: end
...
class V8_EXPORT JsoGenerator {
 public:
  ...
  MaybeLocal<UnboundScript> Compile(ScriptCompiler::Source* source);

  void Generate();

  size_t Output(std::ostream& os);

  void Print();

  void SetCreateFrameworkJso(bool create_framework_jso);
  bool CreateFrameworkJso();
  void SetUseFrameworkJso(bool use_framework_jso);
  bool UseFrameworkJso();
  void RecordJsoHashSeed(uint32_t addr);
  void RecordJsoRange(uintptr_t addr);
...
class V8_EXPORT JsoLoader {
 public:
  ...
  int Load(int fd, bool invertedReplaceFlag = false, bool loadFramework = false);

  // Find aot script by name
  // isolate: current isolate
  // name:   aot script's filename
  // Return:    MaybeLocal<UnboundScript>
  static MaybeLocal<Value> GetUnboundScript(Isolate* isolate, std::string name);

  // Convert Local<Value> to Local<UnboundScript>
  // script should a SharedFunctionInfo
  static MaybeLocal<UnboundScript> Convert(Local<Value> script);

  // Fetch JsoRange from <preload>.jso, which record the target JsoRange.
  static uintptr_t FetchJsoRangeFromPreload(const char* preload_jso_path = nullptr);
...
class V8_EXPORT JSON {
 public:
  ...
  // <ALIOS> adapt for webengin 5.7: begin
  V8_DEPRECATE_SOON("Use the maybe version taking context")
...
// <ALIOS> adapt for webengin 5.7: end
  ...
  V8_DEPRECATE_SOON("Use maybe version") inline Local<Boolean> ToBoolean() const;
  V8_DEPRECATE_SOON("Use maybe version") inline Local<String> ToString() const;
  V8_DEPRECATE_SOON("Use maybe version") inline Local<Object> ToObject() const;
  V8_DEPRECATE_SOON("Use maybe version")
                     inline Local<Integer> ToInteger() const;
  V8_DEPRECATE_SOON("Use maybe version") inline Local<Number> ToNumber() const;
  V8_DEPRECATE_SOON("Use maybe version")
                     inline Local<String> ToDetailString() const;
  V8_DEPRECATE_SOON("Use maybe version") inline Local<Uint32> ToUint32() const;
  V8_DEPRECATE_SOON("Use maybe version") inline Local<Int32> ToInt32() const;
  V8_DEPRECATE_SOON("Use maybe version")
...
class ReturnValue {
 public:
  ...
  // <ALIOS> adapt for webengin 5.7: begin
  template <typename S>
  V8_DEPRECATE_SOON("Use Global<> instead")
...
class FunctionCallbackInfo {
 public:
  ...
  V8_DEPRECATED("Use Data() to explicitly pass Callee instead")
...
class V8_EXPORT ObjectTemplate : public Template {
 public:
  ...
  // <ALIOS> adapt for webengin 5.7: begin
  V8_DEPRECATE_SOON("Use isolate version") static Local<ObjectTemplate> New();
...
class V8_EXPORT ResourceConstraints {
 public:
  ...
  // <YUNOS> changed: begin
  size_t max_old_space_capacity() const { return max_old_space_capacity_; }
  void set_max_old_space_capacity(size_t value) { max_old_space_capacity_ = value; }
  // <YUNOS> changed: end
 ...
 private:
  ...
  // <YUNOS> changed: begin
  size_t max_old_space_capacity_ = 0;
  // <YUNOS> changed: end
};
...
class V8_EXPORT EmbedderHeapTracer {
 public:
  ...
  // <AliOS> change: end
  // adapt for v8 7.7, do same as 7.7
  // virtual void TracePrologue(TraceFlags flags) {}
   ...
   */
  V8_DEPRECATE_SOON("Use IsTracingDone") virtual size_t NumberOfWrappersToTrace() {
    return 0;
  }
  // <ALIOS> adapt for webengin 5.7: end
...
class V8_EXPORT Isolate {
 public:
  ...
  struct CreateParams {
    ...
    // <AliOS> changed: begin
    // only consume in Ioslate::New
    uintptr_t jso_range_start_addr;

    // only consume in Ioslate::New
    size_t jso_range_size;
    // <AliOS> changed: end
  };
  ...
  // <YUNOS> changed: begin
  // To work arround the latency issue of running of Promise/Microtask,
  // we post a ForegoundTask of processing Microtasks so that these microtasks
  // can get running just after Function::Call().
  // Further more, Function::Call() from node::MakeCallback() won't either run
  // microtasks or post MicrotasksRunTask because microtasks is suppressed by
  // SuppressMicrotaskPendingScope in node::MakeCallback().
  /**
   * Do not post any foreground task to run microtasks while this scope is active.
   */
  class V8_EXPORT SuppressMicrotaskPendingScope {
   public:
    explicit SuppressMicrotaskPendingScope(Isolate* isolate);
    ~SuppressMicrotaskPendingScope();

    // Prevent copying of Scope objects.
    SuppressMicrotaskPendingScope(const SuppressMicrotaskPendingScope&) =
        delete;
    SuppressMicrotaskPendingScope& operator=(
        const SuppressMicrotaskPendingScope&) = delete;
   ...
   */
// <YUNOS> changed: begin
  static Isolate* Allocate(bool enable_jso_range = false);
// <YUNOS> changed: end
...
// <YUNOS> changed: end
  ...
  // <ALIOS> adapt for webengin 5.7: begin
  /**
   * Set gc profile file saved directory {PATH}
   * profile path will be {PATH}/v8-gc-profile-{TID}.json
   */
  void SetGCProfileDIR(const char* dir);

  /**
   * Get JS backtrace string, thread safe but not signal safe. Should provide
   * v8::RegisterState in target thread and the thread MUST be paused and waite
   * until this function return. Return string should deleted by caller.
   */
  char* GetBackTraceStringSafe(RegisterState* regs, size_t frames_limit);
  /**
   * Fill v8::RegisterState in current JS thread, signal safe.
   */
  ...
  */
  ...
  StateTag CurrentVMState();
  bool IsNativeVMThreadState();

  /**
   * select coverage mode to certain kind and coverage start to collect coverage
   */
  void StartCollectCoverage(bool call_count = true, bool detailed = true);
  /**
   * stop collect coverage and output to a file
   */
  void StopCollectCoverage(const char* path);
  /**
   * stop collect coverage and output to a JS string, the coverage only include
   * script path, function name, start/end offset
   */
  Local<String> StopCollectSimpleCoverage();
  /**
   * collect all scripts and functions in JS heap
   */
  void CollectAllCoverageData(const char* path);
  // <ALIOS> adapt for webengin 5.7: end
...
class V8_EXPORT V8 {
  ...
  */
   ...
   */
  V8_DEPRECATE_SOON("The natives blob is deprecated (https://crbug.com/v8/7624).")
  ...
  // <YUNOS> changed: begin
  /** Get the AliOS version string. */
  static const char* GetAliOSVersion();


  /** Get the XTS version string. */
  static const char* GetXTSVersion();


  /** Get the JSAOT version string. */
  static const char* GetJSAOTVersion();


  /** Get the VEX version string. */
  static const char* GetVEXVersion();

  /**
  *
  */
  typedef int32_t (*AtomicsExNotifier) (Isolate*, void*, size_t, uint32_t);
  static void SetAtomicsExNotifier(AtomicsExNotifier notifier);
  // <YUNOS> changed: end
...
// <YUNOS> changed: begin
   ...
   */
  static bool InitializeJsoRange(uintptr_t raw_addr, size_t size, Isolate* isolate);
#endif  // ENABLE_JSAOT_
// <YUNOS> changed: end
...
void Isolate::SetObjectGroupId(const Persistent<T>& object,
                               UniqueId id) {
  fprintf(stderr, "TODO: before abort: %s at line %d\n", __FUNCTION__, __LINE__);
  abort();
}
